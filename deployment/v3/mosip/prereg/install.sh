...
helm -n $NS install prereg-application mosip/prereg-application --version $CHART_VERSION --set image.repository=mosipmec/pre-registration-application-service --set image.tag=mec-prod-1.2.0.1-B1
...
helm -n $NS install prereg-datasync mosip/prereg-datasync --version $CHART_VERSION --set image.repository=mosipmec/pre-registration-datasync-service --set image.tag=mec-prod-1.2.0.1-B1
...
helm -n $NS install prereg-ui mosip/prereg-ui --set prereg.apiHost=$PREREG_HOST --version $CHART_VERSION --set image.repository=mosipmec/pre-registration-ui --set image.tag=mec-prod-1.2.0.1-B1
